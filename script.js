...
function descargarPlantillaCSV(){
  ...
  var ejemplos = [
    ...
    },
    {
      ...
      pedidos_nuevos_actual:620, pedidos_nuevos_base:590, pedidos_recurrentes_actual:870, pedidos_recurrentes_base:840,
      frecuencia_actual:1.6, frecuencia_base:1.58, pedidos_totales_actual:1490, pedidos_totales_base:1430,
      meta_pedidos:1600, real_pedidos:1490, meta_sesiones:420000, real_sesiones:410000
    },
    {
      canal:'whatsapp', meta:980000, dias_transcurridos:31, dias_totales:31, venta_actual:915000,
      venta_yoy:860000, venta_mom:900000,
      cr_actual:38, cr_base:41, aov_actual:2210, aov_base:2185,
      pedidos_nuevos_actual:140, pedidos_nuevos_base:150, pedidos_recurrentes_actual:275, pedidos_recurrentes_base:260,
      frecuencia_actual:1.9, frecuencia_base:1.85, pedidos_totales_actual:415, pedidos_totales_base:410,
      meta_pedidos:445, real_pedidos:415, meta_sesiones:1100, real_sesiones:1050
    },
    {
      canal:'llamadas', meta:610000, dias_transcurridos:31, dias_totales:31, venta_actual:545000,
      venta_yoy:590000, venta_mom:570000,
      cr_actual:22, cr_base:25, aov_actual:2480, aov_base:2460,
      pedidos_nuevos_actual:60, pedidos_nuevos_base:75, pedidos_recurrentes_actual:160, pedidos_recurrentes_base:165,
      frecuencia_actual:1.4, frecuencia_base:1.45, pedidos_totales_actual:220, pedidos_totales_base:240,
      meta_pedidos:250, real_pedidos:220, meta_sesiones:980, real_sesiones:920
    }
  ];
  var filas = ejemplos.map(function(ejemplo){
...
async function llamarCohere(apiKey, prompt){
...
document.getElementById('btn-generar-gemini').addEventListener(
  ...
  async function(){
    ...
    try {
      ...
      var resultado = await llamarCohere(apiKeyInput, prompt);
...
var chatIAHistorial = [];
var chatIASistemaPrompt = null;
var chatIAMsgSeq = 0;

function construirSistemaChatIA(contexto){
  return "Sos el asistente de investigación de un analista senior de Ecommerce, dentro de una herramienta de diagnóstico de brecha de ventas. Respondés preguntas sobre el diagnóstico actual: qué está pasando, qué podría explicarlo, y qué probar o investigar a continuación.\n\n" +
    "REGLAS QUE NUNCA ROMPÉS:\n" +
    "- NO determinás la causa definitiva de nada, NO cerrás el diagnóstico.\n" +
    "- NO inventás eventos, cambios, campañas, porcentajes, montos o métricas que no estén en el contexto de abajo.\n" +
    "- Distinguís siempre: DRIVER (variable matemática que explica la brecha) ≠ SEÑAL (comportamiento detectado en Nivel 2) ≠ MECANISMO (cómo podría estar pasando) ≠ HIPÓTESIS (posible explicación) ≠ CAUSA (hipótesis ya validada con evidencia).\n" +
    ...
    "- Usá siempre lenguaje condicional ('podría', 'es posible que', 'sugiere investigar si'). Nunca 'se debe a', 'la causa es', 'confirma que'.\n" +
    "- Si te piden una acción concreta ('hacé esto', 'probá esto'), formulala como algo a probar o investigar e indicá qué dato confirmaría o descartaría esa línea — no como algo ya comprobado.\n" +
...
function agregarMensajeChatIAWelcome(){
  ...
  div.textContent = 'Preguntame sobre el diagnóstico actual del canal seleccionado — qué podría explicar una caída, qué probar, qué investigar primero.';
  cont.appendChild(div);
}
function nuevaConversacionIA(){
  chatIAHistorial = [];
  chatIASistemaPrompt = null;
...
async function enviarMensajeChatIA(){
  ...
  if(!chatIASistemaPrompt){
    var r0 = computeAll();
    chatIASistemaPrompt = construirSistemaChatIA(construirContextoGemini(r0));
  }
  ...
  try {
    ...
    var mensajesParaEnviar = [{role:'system', content: chatIASistemaPrompt}].concat(chatIAHistorial);

    var response = await fetch("https://api.cohere.com/v2/chat", {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + apiKeyInput },
      body: JSON.stringify({ model: 'command-a-plus-05-2026', messages: mensajesParaEnviar, temperature: 0.4 })
    });
