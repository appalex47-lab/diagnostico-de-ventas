...
var CSV_FIELD_MAP = {
  ...
  'meta_pedidos':'inp-meta-pedidos', 'real_pedidos':'inp-real-pedidos',
  'meta_sesiones':'inp-meta-sesiones', 'real_sesiones':'inp-real-sesiones'
};
...
function computeAll(){
  ...
  var metaPedidosPacing=num('inp-meta-pedidos'), realPedidosPacing=num('inp-real-pedidos');
  ...
  var metaSesionesPacing=num('inp-meta-sesiones'), realSesionesPacing=num('inp-real-sesiones');
...
function calcularSnapshotsPorCanal(){
  ...
  canales.forEach(function(c){
    ...
    snaps.push({
      ...
      metaPedidos:num('inp-meta-pedidos'), realPedidos:num('inp-real-pedidos'),
...
function renderVistaGlobal(){
  ...
  document.getElementById('g-insights-canal').innerHTML = ordenados.map(function(s){
    var driver = s.r.primary ? s.r.primary.label : 'Sin driver negativo';
    var brechaTxt = s.r.brechaPct===null ? 'sin pacing cargado' : fmtPct(s.r.brechaPct)+' vs. esperado';
    return '<div class="out"><span class="l">'+(CANAL_NAMES[s.canal]||s.canal)+'</span>'
      + '<span class="v" style="color:'+(COLOR_ESTADO[s.r.estadoClass]||'var(--ink)')+';">'+s.r.estado+' ('+brechaTxt+')</span></div>'
      + '<p class="chk-why" style="margin:-4px 0 10px;">Driver: '+driver+'</p>';
  }).join('');
}
...
function cargarEjemplo(){
  ...
  document.getElementById('inp-meta-pedidos').value=4300;
  document.getElementById('inp-real-pedidos').value=4020;
  document.getElementById('inp-meta-sesiones').value=1950000;
  document.getElementById('inp-real-sesiones').value=1830000;
  ...
  document.getElementById('inp-pt-a').value=4020;
  document.getElementById('inp-pt-b').value=4310;
...
function descargarPlantillaCSV(){
  var headers = ['canal'].concat(Object.keys(CSV_FIELD_MAP));
  var ejemplo = {
    ...
    meta_pedidos:4300, real_pedidos:4020, meta_sesiones:1950000, real_sesiones:1830000
  };
  var fila = headers.map(function(h){ return ejemplo[h]!==undefined ? ejemplo[h] : ''; });
...
function importarCSV(file){
  ...
  reader.onload = function(e){
    try {
      ...
      document.getElementById('form-canal').style.display='';
      document.getElementById('vista-global').style.display='none';
      canalActual = document.getElementById('sel-canal').value;
      try { localStorage.setItem('canal-actual', canalActual); } catch(e){}
      actualizarLabelsCanal();
      recalc();
      cargarHistorial();
      guardarPeriodoCompartido();
...
actualizarLabelsCanal();
aplicarModo();
if(canalActual==='global'){ renderVistaGlobal(); } else { recalc(); }

// --- INTEGRACIÓN CON GEMINI: GENERACIÓN DE HIPÓTESIS ---

// ADVERTENCIA DE SEGURIDAD: esta key queda visible en texto plano para
// cualquiera que abra "Ver código fuente" o las herramientas de desarrollador
// del navegador, y en el historial de Git si este archivo se versiona.
...
// detectarSenales(r) para las señales de Nivel 2 y generarHipotesis(r) para
// las hipótesis que el propio sistema (basado en reglas) ya sugiere.
function construirContextoGemini(r){
  var lineas = [];
  lineas.push('=== DIAGNÓSTICO GENERAL Y NIVEL 1 ===');
  lineas.push(buildReporte(r));
  lineas.push('Sesiones/Interacciones: '+num('inp-ses-a')+' (actual) vs. '+num('inp-ses-b')+' (base) — impacto '+fmtMoney(r.efSes)+' ('+(r.pctSes===null||r.pctSes===undefined?'—':fmtPct(r.pctSes))+' de la variación).');
  lineas.push('CR/Tasa de cierre: '+num('inp-cr-a')+'% (actual) vs. '+num('inp-cr-b')+'% (base) — impacto '+fmtMoney(r.efCR)+' ('+(r.pctCR===null||r.pctCR===undefined?'—':fmtPct(r.pctCR))+' de la variación).');
  lineas.push('AOV: '+fmtMoney(num('inp-aov-a'))+' (actual) vs. '+fmtMoney(num('inp-aov-b'))+' (base) — impacto '+fmtMoney(r.efAOV)+' ('+(r.pctAOV===null||r.pctAOV===undefined?'—':fmtPct(r.pctAOV))+' de la variación).');
  lineas.push('Driver matemático principal: '+(r.primary?r.primary.label:'sin driver dominante')+'.');

  lineas.push('');
  lineas.push('=== NIVEL 2 — SEÑALES DETECTADAS ===');
  var senales = detectarSenales(r);
  if(senales.length){
    senales.forEach(function(s){
      lineas.push('- '+s.label+' | Revisar: '+s.revisar+' | Por qué es señal: '+s.porQue);
    });
  } else {
    lineas.push('No se detectaron señales de Nivel 2 más allá del driver principal.');
  }

  lineas.push('');
  lineas.push('=== HIPÓTESIS YA SUGERIDAS POR EL SISTEMA (motor de reglas existente) ===');
  var hipsSistema = generarHipotesis(r);
  if(hipsSistema.length){
    hipsSistema.forEach(function(h,i){ lineas.push((i+1)+'. '+h); });
  } else {
    lineas.push('El sistema todavía no generó hipótesis propias (faltan señales suficientes).');
  }

  return lineas.join('\n');
}

// Generar hipótesis utilizando todo el contexto disponible
document.getElementById('btn-generar-gemini').addEventListener('click', async function() {

  var btn = this;
  var respuestaDiv = document.getElementById('gemini-hipotesis-respuesta');

  if (!GEMINI_API_KEY || GEMINI_API_KEY === 'PEGA_AQUI_TU_API_KEY') {
    respuestaDiv.style.display = 'block';
    respuestaDiv.innerHTML =
      '<b>No fue posible generar hipótesis con Gemini.</b><br>' +
      '<span style="color:var(--danger);">Falta configurar la API key: reemplazá el valor de GEMINI_API_KEY en el código.</span>';
    return;
  }

  btn.textContent = 'Analizando diagnóstico...';
  btn.disabled = true;

  respuestaDiv.style.display = 'block';
  respuestaDiv.innerHTML =
    '<span style="color:var(--ink-muted);">Analizando diagnóstico, señales y contexto...</span>';

  try {

    // Obtener el diagnóstico completo utilizando las funciones existentes.
    var r = computeAll();
    var contexto = construirContextoGemini(r);

    // Construir contexto para Gemini.
    var prompt =
      "Eres un asistente especializado en diagnóstico de Ecommerce.\n\n" +

      "Tu función NO es determinar la causa definitiva.\n" +
      "Tu función es generar hipótesis investigables a partir del diagnóstico proporcionado.\n\n" +

      "Diferencia claramente:\n" +
      "- Driver = variable matemática que explica la brecha de ventas.\n" +
      "- Señal = comportamiento relevante detectado en Nivel 2.\n" +
      "- Hipótesis = posible explicación que relaciona una o más señales.\n" +
      "- Causa = hipótesis que posteriormente será validada con evidencia.\n\n" +

      "Nunca presentes una hipótesis como una causa confirmada. " +
      "Una señal secundaria debe mantenerse relevante aunque el driver principal sea otra variable — no la descartes solo por eso.\n\n" +

      "=== CONTEXTO DEL DIAGNÓSTICO ===\n" +
      contexto + "\n\n" +

      "=== TAREA ===\n" +
      "Genera entre 3 y 5 hipótesis de negocio específicas y accionables. Para cada una:\n" +
      "1. Hipótesis (explicación concreta)\n" +
      "2. Señales relacionadas (cuáles señales del contexto la respaldan)\n" +
      "3. Qué investigar (dato, segmento o comparación exacta a revisar)\n\n" +

      "No inventes datos ni porcentajes que no estén en el contexto de arriba. " +
      "Evita hipótesis genéricas y no repitas la misma hipótesis dos veces. " +
      "Si la información disponible no permite una hipótesis suficientemente específica, dilo y señala qué dato falta.\n\n" +
      ...
      "Investigar:\n" +
      "• ...\n\n" +
      "(repetir para H2, H3 y, si hay información suficiente, H4 y H5)";
    ...
    if (!response.ok) {
      throw new Error(
        (response.status || '?') + ' — ' + (data.error ? data.error.message : 'Error en la petición a Gemini')
      );
    }

    var texto = data.candidates && data.candidates.length > 0
    ...
    if (texto) {
      ...
      respuestaDiv.innerHTML =
        '<b style="color:var(--accent);">🤖 Hipótesis sugeridas por Gemini:</b><br><br>' +
        texto +
        '<p style="margin-top:12px;font-size:11px;color:var(--ink-muted);">Estas son hipótesis generadas a partir de las señales disponibles. Todas requieren validación con evidencia antes de considerarse una causa.</p>';

    } else {

      respuestaDiv.innerHTML =
        '<b>No fue posible generar hipótesis con Gemini.</b><br>' +
        '<span style="color:var(--danger);">La respuesta no incluyó texto utilizable (sin "candidates" o sin contenido).</span>';
    ...
    respuestaDiv.innerHTML =
      '<b>No fue posible generar hipótesis con Gemini.</b><br>' +
      '<span style="color:var(--danger);"><b>Error:</b> ' + error.message + '</span><br>' +
      '<span style="font-size:11px;color:var(--ink-muted);">Revisa: API key, modelo utilizado, conexión, o la respuesta de la API.</span>';

  } finally {

    btn.textContent = '🤖 Generar hipótesis con Gemini AI';
    btn.disabled = false;

  }
});

// --- FIN INTEGRACIÓN CON GEMINI ---
